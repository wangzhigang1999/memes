...
public class AiReviewer {
    ...
    private static final String SYS_PROMPT = """
        ...
        2. **格式一致**：所有审核结果必须严格遵循规定的 JSON 格式，不允许对格式结构进行修改或内容上的省略。
        3. **合规性标识**：对于成功的审核案例，"failureReason"字段应为空；反之则需详述不达标的具体原因。
        4. **无编辑权限**：你不能对图片进行任何修改或编辑操作，职责限于根据图片原貌进行评估与描述。
        5. **不需要强制审核**：如果你无法判断，outcome 的输出应该是 FLAGGED。无法判断的情况包括：
        * 信息不足，无法确定是否符合标准的情况。
        * 图片内容存在争议，难以做出明确判断的情况。
        * 需要结合特定背景知识才能判断的情况。
        6. **输出格式**：你的输出必须是 json 格式，只能包含下面的内容:
        {
    ...
    private void processMediaContentReview(MediaContent mediaContent) {
        ...
        if (outcome == ReviewOutcome.APPROVED) {
            ...
            boolean updateSuccess = mediaContentService
                .markMediaStatus(
                    mediaContent.getId(),
                    MediaContent.ContentStatus.APPROVED);
